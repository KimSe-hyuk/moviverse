...
<img style="position: fixed;    margin-left: 1700px;     width: 70px;     cursor: pointer;margin-top: 740px;"  onClick="javascript:window.scrollTo(0,0)"src="img/up.png">
            <?php 
if(	$_SESSION["userId"]!=""){
...
<?php
	...
	$query3 = $db->query("select * FROM genres where genre_id='$bid'");
	if($bid==""){
	?>
			    <div class="movie_all">
...
?>
        		    <div class="movie_all">
                <p class="movietext">영화ㅣ <?=$row['genres_name']?> </p>
<?php
}
?>
                <script>
                ...
                <p id="dok"style="color:black;">					    <img   src="img/filter.png  ">독점작</p>
...
<img onclick="location.href='movie.php?bid='" class="where_logo"src="img/all.png">
                ...
                <a style="color:black;"id="carta" href="javascript:doDisplay();">카테고리▼</a>
                ...
                <div style="display: none;" id="myDIV">
                    ...
                    <p id="pop"><a style="color: <?php 	if($Popularity_Newest=="popularity"){echo'#3482EA';}else{echo'#BDBDBD';}?>;"href="movie.php?Popularity_Newest=popularity&bid=<?=$bid?>&search=<?=$search?>&platform=<?=$platform?>">인기순</a></p>
 
                    <p id="now"><a style="color:  <?php if($Popularity_Newest!="popularity"){echo '#3482EA';}else{echo'#BDBDBD';}?>;" href="movie.php?Popularity_Newest=release_date&bid=<?=$bid?>&search=<?=$search?>&platform=<?=$platform?>">최신순</a></p>
